...
var CommitLayout = (key) => {
  return `<div style='margin-top:10px'>{{${C1.key+'_'+key}}}</div><div style='margin-top:10px;margin-bottom:20px;'>{{${C2.key+'_'+key}}}</div>`
}
...
var QN2 = {
  key: 'mgr2mem',
  label: '项目/部门经理->员工',
  questions: [Q1, Q5, Q9, Q6, Q7]
}
var QN3 = {
  key: 'dep2mem',
  label: '事业部->员工',
  condition: `v-if="db.model.position!=0"`,
  questions: [Q1, Q5, Q9, Q6, Q7, Q10, Q11]
}
var QN4 = {
  key: 'dep2mgr',
  label: '事业部->项目/部门经理',
  condition: `v-else`,
  questions: [Q1, Q4, Q2, Q3, Q7, Q10, Q11]
}
...
var flow = {
  ...
  nodes: {
    ...
    n3: {
      ...
      layout: `
      <div><h2 style='text-align:center;font-family:仿宋;font-size:20px;font-weight:bold;'>项目部打分评价表(平行责任人)</h2>
      <template v-if='db.model.executors[2].length > 0'>
      ${CreateENLayout('n31')}
      ${CommitLayout('n31')}
        ${MAKE_CONDITION_LAYOUTS([QN2],"1")}
        </template><template v-else='db.model.executors[2].length > 2'>
      ${CreateENLayout('n32')}
      ${CommitLayout('n32')}
        ${MAKE_CONDITION_LAYOUTS([QN2],"2")}
        < /template><template v-else='db.model.executors[2].length > 3'>
      ${CreateENLayout('n33')}
      ${CommitLayout('n33')}
      ...
      <Row :gutter='12' style='margin-top:30px'><Col :span='5'>{{name}}</Col><Col :span='9'>{{dep}}</Col><Col :span='10'>{{position}}</Col></Row>
       ...
       <div style='margin-top:5px'>{{project}}</div>
      <div style='margin-top:10px'>{{report}}</div>
      <h2 style='text-align:center;font-family:仿宋;font-size:20px;font-weight:bold;'>项目部打分评价表(第一责任人)</h2>
      ${CreateENLayout('n2')}
      ${CommitLayout('n2')}
      ${MAKE_CONDITION_LAYOUTS([QN2])}
     <h2 style='text-align:center;font-family:仿宋;font-size:20px;font-weight:bold;'>员工自评表</h2> 
    ...
    n4: {
      ...
      <Row :gutter='12' style='margin-top:30px'><Col :span='5'>{{name}}</Col><Col :span='9'>{{dep}}</Col><Col :span='10'>{{position}}</Col></Row>
       ...
       <div style='margin-top:5px'>{{project}}</div>
      <div style='margin-top:10px'>{{report}}</div>
      <h2 style='text-align:center;font-family:仿宋;font-size:20px;font-weight:bold;'>项目部打分评价表(平行责任人)</h2>
     <template v-if='db.model.executors[2].length > 0'>
      ${CreateENLayout('n31')}
      ${CommitLayout('n31')}
        ${MAKE_CONDITION_LAYOUTS([QN2],"1")}
    ...
    n5: {
      ...
      ${CreateENLayout('n4')}
       ${CommitLayout('n4')}
      ${MAKE_CONDITION_LAYOUTS([QN3,QN4])}
       <h2 style ='text-align:center;font-family:仿宋;font-size:20px;font-weight:bold;' > 事业部打分(平行责任人) 打分结果</h2>
       <template v-if='db.model.executors[2].length > 0'>
      ${CreateENLayout('n31')}
      ${CommitLayout('n31')}
        ${MAKE_CONDITION_LAYOUTS([QN2],"1")}
