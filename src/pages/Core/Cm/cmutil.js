...
out.install = v => {
  for (let x in out) {
    if (typeof out[x] === 'object')
      v.component(x, out[x])
    else if (typeof out[x] === 'function')
      if (v.prototype.util)
...
out.formatSalarySemicolon = (salary) => {
  ...
  return (nagivate ? '- ' : '') + output + fraction
}
...
out.formatSalary = (salary) => {
  ...
  var num = salary ? parseInt(salary) : 0
  var output = ""
  const units = [" ", " 万", " 亿"]
  let count = 0
  while (num >= 10000) {
...
out.renderFullAmount = (h, amount, highlight, center) => {
  ...
  return h("div", {
    ...
    style: {
      color: amount == 0 ? '#ddd' : (highlight ? highlight : "#888"),
      marginLeft: "0.2rem",
...
out.renderDateSimple = (h, d, hl, vnow) => {
  if (typeof d == 'string' && d != "null" && d != "") {
    ...
    return h('div', {
      ...
      style: {
        color: hl ? 'blue' : (isFuture ? 'orange' : '#777'),
        textAlign: 'cener',
        fontSize: '0.7rem',
        lineHeight: '1rem'
      }
...
out.renderDateTime = (h, d, highlight) => {
  if (typeof d == 'string' && d != "null" && d != "") {
    let m = d.includes('T') ? moment(d) : moment(d, 'YYYY-MM-DD')
    let dateString = m.format('YYYY/MM/DD HH:mm:ss')
    let dateSmart = m.fromNow()
    if (dateSmart.includes('小时'))
      dateSmart = '1天内'
    return h('div', {
...
out.renderComparePercent = (h, d, percent, colorReverse, renderFunc, inline) => {
  if (d || percent) {
    ...
    }, [out.renderCompareMark(h, percent, colorReverse), (percent ? Math.abs(percent * 100).toFixed(1) : 0) + '%']))
    return h('div', {
      style: inline ? styleInline : styleNotInline
    }, renderItems)
  } else {
...
out.renderProgressValue = (h, percent, value, color = 'linear-gradient(to bottom,#d6f4f8,#92cdf3)', bgcolor = 'linear-gradient(to bottom,#eee,#fff)') => {
...
out.renderCompareProgress = (h, percent, r) => {
  let percentItems = []
  let colorOver = "linear-gradient(to bottom,#fa3833,#fa5853)"
  let colorDrawn = 'linear-gradient(to bottom,#5cb515,#6cc525)'
  if (r) {
    colorOver = 'linear-gradient(to bottom,#5cb515,#6cc525)'
    colorDrawn = "linear-gradient(to bottom,#fa3833,#fa5853)"
  }
  if (typeof percent != 'undefined')
    percentItems.push(h("div", {
      style: {
        ...
        background: percent == 100 ? "linear-gradient(to bottom,#5cb515,#6cc525)" : (percent < 100 ? colorDrawn : colorOver),
        width: (percent && percent > 100 ? 100 : (percent || 0)) + '%'
      }
    }))
  else
  ...
  if (typeof percent != 'undefined')
    percentItems.push(h("div", {
      style: {
        ...
        zIndex: '0',
        transition: 'all 0.3s',
        background: 'linear-gradient(to bottom,#3f8bc1,#5fabf1)',
        width: (percent && percent > 100 ? 100 : (percent || 0)) + '%'
      }
    }))
  else
...
out.modifyDatePicker = (parent, key) => {
  let d = parent[key]
  if (d) {
