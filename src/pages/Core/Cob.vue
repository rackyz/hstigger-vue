<template>
  <Layout class="hs-container hs-container-full statistics" >
    ...
    <div style="height:calc(100% - 30px);position:relative;">
      <hs-table ref="table" :columns="cols" bordered :data="filteredItems" @event="onTableEvent" selectable="false" :option="{summary:true}" />
    </div>
    </Content>
  ...
  methods:{
    ...
    handleDelete(model){
      this.Confirm(`确定删除该资料<b style='color:red;margin:0 2px;'>${model.name}</b>及相关文件`,()=>{
        this.api.enterprise.DELETE_BIDORDERS({param:{id:model.id}}).then(res=>{
          setTimeout(() => {
            this.Success('删除成功')
    ...
    handlePatchArchive(item){
      if(item.id){
        let id = item.id
        delete item.id
        this.api.enterprise.PATCH_BIDORDERS(item,{param:{id}}).then(res=>{
        ...
        this.api.enterprise.POST_BIDORDERS(item).then(res=>{
          let updateInfo = res.data.data
          let new_item = Object.assign({},item,updateInfo)
    ...
    getData(){
       this.loading = true
       this.api.enterprise.LIST_BIDORDERS().then(res=>{
